...
#my $cmd  = "cd /bioseq/data/results/oneTwoTree/test_job;perl $serverName.pl --id $jobId --dir $curJobdir --file $zipServerLocation ";

`perl /bioseq/OTT_dev/sendFirstEmail.pl --jobTitle "$jobTitle" --toEmail $email_to_address --id $jobId`;
...
if( $pid == 0 )
{
	...
	#print QSUB_SH "perl /bioseq/OTT_dev/sendFirstEmail.pl --toEmail $email_to_address --id $jobId\n";

	my $cmd .= "python /bioseq/OTT_dev/OneTwoTree.py $fnameUserInput $curJobdir $jobId;";
	print QSUB_SH "$cmd\n";
	...
	#`perl /bioseq/OTT_dev/sendFirstEmail.pl --toEmail $email_to_address --id $jobId`;
